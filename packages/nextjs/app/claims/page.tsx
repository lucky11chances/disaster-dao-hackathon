...
import { CheckCircleIcon, ClockIcon, ShieldCheckIcon, XCircleIcon } from "@heroicons/react/24/outline";
...
const CreateClaimForm = () => {
  ...
  const handleSubmit = async () => {
    ...
    const end = impactEnd
      ? BigInt(Math.floor(new Date(impactEnd).getTime() / 1000))
    ...
    <div className="card bg-base-100 shadow-xl">
      <div className="card-body">
        ...
        <div className="form-control mb-3">
          ...
          />
          <label className="label">
            <span className="label-text-alt opacity-50">Address that will receive funding</span>
          </label>
        </div>

        <div className="form-control mb-3">
          <label className="label">
            <span className="label-text font-semibold">Impact Scope *</span>
          </label>
          <input
            type="text"
        ...
        <div className="form-control mb-3">
          <label className="label">
            <span className="label-text font-semibold">Evidence URI</span>
          </label>
          <input
            type="text"
          ...
          />
          <label className="label">
            <span className="label-text-alt opacity-50">Link to receipts, photos, delivery logs</span>
          </label>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div className="form-control">
            <label className="label">
              <span className="label-text font-semibold">Impact Start</span>
            </label>
            <input
              type="datetime-local"
              className="input input-bordered"
          ...
          <div className="form-control">
            <label className="label">
              <span className="label-text font-semibold">Impact End</span>
            </label>
            <input
              type="datetime-local"
              className="input input-bordered"
    ...
    <div className="card bg-base-100 shadow-md border border-base-300">
      <div className="card-body py-4">
        ...
        <div className="text-xs opacity-60 space-y-1 mt-1">
          ...
          <p>
            Recipient: {recipient?.toString().slice(0, 6)}...{recipient?.toString().slice(-4)}
          </p>
          {tokenURI && (
            <p>
              Evidence:{" "}
              <a href={tokenURI?.toString()} target="_blank" rel="noreferrer" className="link link-primary">
                {tokenURI?.toString().slice(0, 40)}...
              </a>
            </p>
          )}
          <p>
            Period: {new Date(Number(impactStart) * 1000).toLocaleDateString()} —{" "}
            {new Date(Number(impactEnd) * 1000).toLocaleDateString()}
          </p>
          <p>
            Evaluations: {passCount?.toString()} passes / {totalEvals?.toString()} total
          </p>
        </div>
        ...
        {isReviewer && statusNum === 0 && !hasEvaluated && (
          <div className="mt-3 border-t border-base-300 pt-3">
            ...
            <div className="flex gap-2">
              ...
              <button className="btn btn-error btn-sm flex-1" onClick={() => handleEvaluate(false)} disabled={isMining}>
