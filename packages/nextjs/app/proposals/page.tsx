    ...
    <div className="flex flex-col items-center grow pt-8 px-4">
      ...
      <div className="flex gap-4 mb-6 flex-wrap justify-center">
        ...
        <div className="badge badge-lg badge-outline">
          💰 Treasury: {treasuryBalance ? parseFloat(formatEther(treasuryBalance)).toFixed(4) : "0"} ETH
        </div>
        <div className="badge badge-lg badge-outline">
          🪙 Your BRDY: {tokenBalance ? parseFloat(formatEther(tokenBalance)).toLocaleString() : "0"}
        </div>
      </div>
      ...
      <div className="card-body">
        ...
        <div className="form-control mb-3">
          <label className="label">
            <span className="label-text font-semibold">Claim ID *</span>
          </label>
          <input
            type="number"
            placeholder="e.g., 1"
          ...
          />
          <label className="label">
            <span className="label-text-alt opacity-50">The approved Impact Claim to fund</span>
          </label>
        </div>
...
const ProposalCard = ({ proposalId }: { proposalId: bigint }) => {
  ...
  const { writeContractAsync: executeAsync, isMining: isExecuting } = useScaffoldWriteContract({
    contractName: "BReadyDAO",
  });
  const { writeContractAsync: cancelAsync, isMining: isCanceling } = useScaffoldWriteContract({
    contractName: "BReadyDAO",
  });
    ...
    <div className="card bg-base-100 shadow-md border border-base-300">
      <div className="card-body py-4">
        ...
        <div className="text-xs opacity-60 space-y-1 mt-1">
          <p>
            Claim: #{pClaimId?.toString()} | Amount: {formatEther(pAmount || 0n)} ETH
          </p>
          <p>
            Proposer: {proposer?.toString().slice(0, 6)}...{proposer?.toString().slice(-4)}
          </p>
          <p>
